<x-app-layout>
    <x-slot name="header">
        <h2 class="font-black text-2xl text-gray-900 uppercase tracking-tight">Data Pelanggan</h2>
    </x-slot>
    ...
    <div class="py-12" x-data="{ modalAdd: false, modalEdit: false, modalDetail: false, editData: {}, detailData: {} }">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white rounded-[30px] shadow-xl border border-gray-100 overflow-hidden">
                <div class="p-8 border-b border-gray-100 flex justify-between items-center bg-indigo-950 text-white">
                    <div>
                        <h3 class="font-black text-xl uppercase tracking-tighter">Daftar Pelanggan</h3>
                        <p class="text-indigo-300 text-xs mt-1 font-medium">{{ $pelanggan->count() }} pelanggan terdaftar.</p>
                    </div>
                ...
                <div class="p-4 border-b border-gray-100 bg-gray-50">
                    <div class="flex gap-3 items-center">
                        ...
                        <input type="text" id="searchPelanggan" placeholder="Cari pelanggan..." class="border-0 bg-transparent text-sm font-medium text-gray-600 placeholder-gray-400 focus:outline-none focus:ring-0 flex-1">
                    </div>
                </div>
                    ...
                    <table class="w-full text-left border-collapse" id="pelangganTable">
                        <thead>
                            <tr class="bg-gray-50 text-gray-400 text-[10px] uppercase tracking-widest">
                                ...
                                <th class="p-5 font-black">Alamat</th>
                                <th class="p-5 font-black text-center">Poin</th>
                                <th class="p-5 font-black text-center">Total Transaksi</th>
                            ...
                            <tr class="hover:bg-gray-50 transition">
                                <td class="p-5">
                                    <div class="flex items-center gap-3">
                                        ...
                                        @endif
                                        <div>
                                            <span class="font-black text-gray-900 uppercase text-sm">{{ $p->name }}</span>
                                            <p class="text-[10px] text-gray-400 font-bold">{{ $p->email }}</p>
                                        </div>
                                    </div>
                                </td>
                                <td class="p-5 text-sm text-gray-600">
                                    <p>{{ $p->telepon ?? '-' }}</p>
                                    <p class="text-[10px] font-bold text-gray-400">{{ $p->jenis_kelamin == 'L' ? 'Laki-laki' : ($p->jenis_kelamin == 'P' ? 'Perempuan' : '') }}</p>
                                </td>
                                <td class="p-5 text-sm text-gray-600">{{ $p->alamat ? \Illuminate\Support\Str::limit($p->alamat, 30) : '-' }}</td>
                                <td class="p-5 text-center">
                                    <span class="font-black text-lg text-amber-600">{{ $p->poin }}</span>
                                </td>
                                <td class="p-5 text-center font-bold text-gray-900">{{ $p->pesanan_count }}x</td>
                                <td class="p-5 text-center">
                                    <div class="flex items-center justify-center gap-2">
                                        <a href="{{ route('admin.pelanggan.show', $p->id) }}" class="p-2 bg-gray-50 text-gray-600 hover:bg-gray-900 hover:text-white rounded-xl transition" title="Detail">
                                        ...
                                        <form action="{{ route('admin.pelanggan.destroy', $p->id) }}" method="POST" onsubmit="return confirm('Hapus pelanggan ini?')">
                                            @csrf @method('DELETE')
        ...
        <div x-show="modalAdd" style="display: none;" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
            <div @click.away="modalAdd = false" class="bg-white p-8 rounded-3xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
                <h3 class="font-black text-gray-900 uppercase text-xl mb-6">Tambah Pelanggan Baru</h3>
                <form action="{{ route('admin.pelanggan.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                    @csrf
        ...
        <div x-show="modalEdit" style="display: none;" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
            <div @click.away="modalEdit = false" class="bg-white p-8 rounded-3xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
                <h3 class="font-black text-gray-900 uppercase text-xl mb-6">Edit Pelanggan</h3>
                <form :action="`{{ url('admin/pelanggan') }}/${editData.id}`" method="POST" enctype="multipart/form-data" class="space-y-4">
                    @csrf @method('PUT')
                    <div class="grid grid-cols-2 gap-4">
                        <div class="col-span-2">
                            <label class="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Nama Lengkap *</label>
                        ...
                        <div>
                            <label class="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Poin</label>
                            <input type="number" name="poin" x-model.number="editData.poin" min="0" class="w-full rounded-xl border-gray-200 focus:ring-indigo-500 font-bold">
                        </div>
                        <div class="col-span-2">
                            <label class="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Alamat</label>
    ...
    <script>
        document.getElementById('searchPelanggan').addEventListener('keyup', function() {
            ...
            document.querySelectorAll('#pelangganTable tbody tr').forEach(function(row) {
